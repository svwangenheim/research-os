...
thank any author that sent us their data.
...
sourced workers1 or undergraduate assistants, often suffer from a limited attention span, fatigue,
...
improves,3 researchers augmenting their text annotation approach with LLMs must always vali-
...
In addition to recommending a standardized process to validate when and how to use LLMs,

we also introduce several novel tools in our automated annotation procedures.5 First, we make
...
codebook)6 and then have at least two subject matter experts and an LLM annotate the same text

samples—with the sample size depending on the type of task and class imbalance.7 Critically, both

the human coders and the LLM should use the same codebook for annotation, where the codebook

serves as the LLM’s prompt instructions.8 Then, the researcher should evaluate performance (i.e.,
...
instructions by emphasizing incorrect classifications (repeating this process if necessary).9 Finally,
...
interdisciplinary publications (e.g., Science Advances and Proceedings of the National Academy of

Sciences) to high-impact field journals in political science (e.g., American Political Science Review

and American Journal of Political Science) and psychology (e.g., Journal of Personality and Social

Psychology).

In the Appendix, Table A1 includes the complete list of replication articles and

Table A2 provides brief descriptions of the annotation tasks in this articles.10 These annotation
...
outreach to authors.11 Whenever possible, we begin with the exact codebook used in the original
...
Figure 2: Precision and recall for each of 27 replicated classification tasks. Color reflects dataset,
such that points sharing the same color are conducted on the same text data.

8

0.000.250.500.751.000.000.250.500.751.00PrecisionRecallscore of 0.06. Moreover, LLM performance varied substantially across tasks within a single dataset.
...
classify each sample at least three times with a temperature above 0.13 For our analyses, we use a
...
negative rate. Given a vector of classifications, C, with length l for a given classification task,
...
i=1 Ci == Cmode). Across tasks, accuracy is 19.4 percentage points higher for text samples
...
50%60%70%80%90%100%0.50.60.70.80.91.0Consistency ScoreProbability of Correct ClassificationAccuracyTrue Negative RateTrue Positive Ratefor LLM classification performance. This step can be thought of as an application of “prompt
...
Figure 4: Change in LLM annotation performance on training data after one round of codebook
...
PrecisionRecallAccuracyF1−0.4−0.20.00.2−0.4−0.20.00.203690369Change in performance metric after codebook updateDensityUse case
...
itize for human review

If a researcher has data that is already labeled by human
...
A researcher can manually review annotations with consis-
...
There are various situations in which a researcher may use
...
In the simplest case, a researcher may find LLM performance
...
cessibility and State Violence
...
The Rise of and Demand for Identity-
...
Informing the Leader: Bureaucracies
...
Framing and blame attribution in pop-
ulist rhetoric
...
The Temporal Focus of Campaign
Communication

People judge others to have more vol-
...
The Impact of Social Identity Conflict
on Planning Horizons
...
Computational analysis of 140 years of
...
Dynamics of cross-platform attention
to retracted papers

Saha et al.

On the rise of fear speech in online so-
cial media 15
...
Code congressional speeches for whether they are about immigra-
...
Code whether tweets express criticism with respect to the findings
of academic papers
...
Code Gab posts as a) fear speech, b) hate speech, or c) normal. Fur-
