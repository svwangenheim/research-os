...
writeLines(tex_output, file.path("04_paper", output_type, "tables", "reg_main_specification.tex"))
```
...
- User uploads a reference PDF to `01_literature/sources/` (or a talk deck to `05_outreach/talks/`)
...
> See also `${CLAUDE_PLUGIN_ROOT}/skills/discover/references/pdf-processing.md` for the discovery-phase variant.
...
1. **Create** — `mkdir -p explorations/[name]/{R,scripts,output}` + README from `${CLAUDE_PLUGIN_ROOT}/templates/exploration-readme.md`
2. **Develop** — work entirely within the exploration folder
3. **Decide:**

   - **Graduate to production** — copy to `03_analysis/scripts/{R,py,jl}/`; requires quality >= 80, tests pass, code clear. Move to `explorations/ARCHIVE/completed_[project]/`
   - **Keep exploring** — document next steps in README
   - **Abandon** — move to `explorations/ARCHIVE/abandoned_[project]/` with explanation (use `${CLAUDE_PLUGIN_ROOT}/templates/archive-readme.md`)
