...
Read `${CLAUDE_PLUGIN_ROOT}/skills/_shared/explorable-contract.md`. The seven clauses are binding; the QA checklist at its end must be completed and included in your final report.
...
1. Write the file to `${ENGRAM_HOME:-$HOME/.claude/learning}/artifacts/<topic>/<node>.html` (create dirs; `mkdir -p` via Bash) — the same home the engine itself resolves (`scripts/engram.py`'s `home()`), so on this install it lands in `<vault root>/_brain/learning/artifacts/...`, Obsidian-visible. Header comment per Contract clause 7 (node id, topic, date, interests used, scaffold level).
