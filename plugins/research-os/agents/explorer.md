...
Paths follow `${CLAUDE_PLUGIN_ROOT}/rules/folder-map.md`; update outputs in place per `${CLAUDE_PLUGIN_ROOT}/rules/output-discipline.md`; record declared external datasets in `passport.yaml` `data_provenance`.
...
- **From related papers:** data used in the Librarian's bibliography (`01_literature/reviews/`)
...
Write to `02_data/data-sources.md` (ranked list with feasibility grades, fit assessment, and access instructions folded in — **update in place** on re-run, with a `## Changelog` entry). Write data dictionaries for top candidate(s) to `02_data/codebooks/`. Record any declared external datasets in `passport.yaml` `data_provenance`.
