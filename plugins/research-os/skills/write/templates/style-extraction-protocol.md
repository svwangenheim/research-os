...
How to extract a personal writing voice from a corpus of the user's prior papers and produce `00_admin/personal-style-guide.md`.
...
### 4. Write to `00_admin/personal-style-guide.md`
...
Scan the sampled papers for `\cite{}`, `\citet{}`, `\citep{}` commands referencing the author's own prior work. List any citation keys found. Cross-check each against `01_literature/bibliography.bib` in the current project. If any self-citation keys are missing from the bib, include a `## Self-Citation Gaps` appendix in the style guide output listing them — so future `/write` calls don't invent or drop those references.
