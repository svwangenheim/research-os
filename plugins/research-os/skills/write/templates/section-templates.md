...
### `literature_review` — Review / Synthesis paper

Not IMRaD. When the review is systematic, mirror the PRISMA structure produced by `/discover lit systematic`.

1. **Introduction** — the question the review answers, why a synthesis is needed now, scope
2. **Protocol / method** — search strategy, databases, inclusion/exclusion criteria (PICO(S)-style for systematic reviews); PRISMA flow counts
3. **Thematic synthesis** — organized by theme, mechanism, or method (not paper-by-paper). Each subsection states what the body of evidence shows and where it disagrees
4. **Risk-of-bias / quality appraisal** — for systematic reviews, the graded assessment table
5. **Gaps and frontier** — what is unresolved; what the field should do next
6. **Conclusion** — the state of knowledge in 1–2 paragraphs

Draws directly on `01_literature/reviews/<question-slug>.md`. Every claim still enters the `claim_manifest` (INV-22).

### `theory` — Pure theory paper

The formal argument is the contribution (pairs with `/strategize theory`).

1. **Introduction** — the question, the modeling contribution, the main result in words
2. **Environment** — agents, timing, information, primitives
3. **Definitions and assumptions** — numbered, each interpreted in plain language
4. **Results** — propositions/theorems, each stated then proved (proofs in an appendix if long)
...
1. **Introduction** — the phenomenon, why this case is informative, what it illuminates
2. **Background / setting** — the institutional context in depth
3. **Case narrative** — the evidence, in analytic (not merely chronological) order
4. **Analysis** — interpret the case against theory/comparators; state the mechanism
5. **Generalizability** — scope conditions: what travels, what is idiosyncratic
6. **Conclusion** — the lesson and its boundaries

### `conference` — Conference / working paper (short form)

A compressed IMRaD: tighter intro, condensed strategy, headline results only, a short discussion. Enforce the venue's page/word limit up front and cut robustness detail to an appendix. Section backbones are the IMRaD ones above, shortened.
