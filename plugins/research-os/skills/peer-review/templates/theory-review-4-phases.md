...
- [ ] **Comparison to literature.** Stronger/weaker than relevant anchors in `00_admin/domain-profile.md` (Theoretical Foundational References)? Relationship stated.
...
- [ ] Symbols match the `Notation Conventions` table in `00_admin/domain-profile.md`
...
- Foundational anchors are drawn from `00_admin/domain-profile.md` (Theoretical Foundational References) when filled. If the domain profile is empty or silent on the relevant topic, the theorist's fallback defaults apply:
...
Cross-check against `01_literature/bibliography.bib`.
...
Save to `04_paper/reviews/[FILENAME]_theory_review.md`:
