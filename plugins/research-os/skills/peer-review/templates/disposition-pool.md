# Editor: Disposition Pool and Decision-Making

Extracted from `editor.md`. Used by the editor agent for referee assignment and editorial decisions.
...
**Selection rule:** Draw dispositions from the journal's **Referee pool** weights (calibrated from the target journals + referee concerns in `00_admin/domain-profile.md`). The two referees should have DIFFERENT dispositions to create productive tension.
...
## Concern Classification (after referee reports)
...
This is where the editor earns their role:
...
## Referee Assignment Output Format
...
## Anti-Sycophancy & Frame-Lock (R&R rounds and rebuttals)

When a referee re-reviews after the author has responded (R&R `--r2`/`--r3`, or any time the manuscript arrives with a rebuttal), the referee must **hold its frame** and not cave to a confident-sounding response. Sycophantic concession — dropping a valid concern because the author pushed back politely — corrupts the review.

**Rebuttal scoring (per concern).** For each prior concern the author claims to have addressed, the referee scores the author's rebuttal on a **1–5 rebuttal-strength scale**:

| Score | Meaning |
|-------|---------|
| 5 | New evidence/analysis fully resolves the concern; verifiable in the revised manuscript |
| 4 | Strong argument or partial new evidence that materially addresses the *core* of the concern |
| 3 | Plausible but incomplete; addresses the periphery, not the core |
| 2 | Assertion without new evidence; restates the original position |
| 1 | Evasive, misdirects, or concedes nothing while claiming to |

**Concession rule (frame-lock).** The referee marks a concern **Resolved** and concedes the point **only if the rebuttal scores ≥ 4 AND the rebuttal addresses the core critique** (not a reframed easier version). Otherwise the concern stays **Partially resolved** (score 3) or **Not addressed** (score ≤ 2). A polite tone, author seniority, or confident phrasing never raises the score — only evidence in the revised manuscript does.

**Dialogue-health check.** Before finalizing, the referee (and the editor when synthesizing) runs a quick self-audit:
- Am I conceding because the evidence is strong, or because pushing back is uncomfortable? (frame-lock)
- Am I manufacturing new demands to avoid ever accepting? (the opposite failure — perpetual goalpost-moving is also flagged)
- Did the author silently narrow the claim to dodge the concern? (scope-creep in reverse)
- Is each remaining concern still the *same* concern I raised, or a drifted version?

Record, per concern, the rebuttal score (1–5) and the resolution status (Resolved / Partially / Not addressed) in the referee's R&R report. The editor's R&R decision letter reflects only genuinely resolved concerns.
