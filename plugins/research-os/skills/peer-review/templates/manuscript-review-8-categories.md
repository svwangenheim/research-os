...
- Does `passport.yaml` `claim_manifest` cover all numerical/non-trivial claims (INV-22)? If not: -15
...
Enforce all Required items from `${CLAUDE_PLUGIN_ROOT}/rules/working-paper-format.md`:
...
**Only scored when `00_admin/personal-style-guide.md` contains real content (not the template).**
