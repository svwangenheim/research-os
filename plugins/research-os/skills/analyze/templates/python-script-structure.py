...
# Script lives in 03_analysis/scripts/py/, so root is three parents up.
# Replace "academic_paper" with the target output folder under 04_paper/.
ROOT = Path(__file__).resolve().parents[3]  # Adjust depth to reach project root
TABLE_DIR = ROOT / "04_paper" / "academic_paper" / "tables"
FIGURE_DIR = ROOT / "04_paper" / "academic_paper" / "figures"
...
# --- Data --------------------------------------------------------------------
df = pd.read_parquet(ROOT / "02_data" / "cleaned" / "analysis_data.parquet")
# Alternative: df = pd.read_csv(ROOT / "02_data" / "cleaned" / "analysis_data.csv")
...
# import pickle
# with open(ROOT / "03_analysis" / "scripts" / "py" / "output" / "model_fit.pkl", "wb") as f:
