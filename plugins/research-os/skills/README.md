...
- **Knowledge:** `wiki-setup`, `add-thematic-wiki`, `wiki-pull`, `wiki-push`, `wiki-ingest`, `wiki-maintain`, `connect` (the wiki skills were vendored here — previously split across a separate `claude-global/` sync mechanism, retired once research-os itself became the global plugin install; `connect` is the read-only cross-theme bridge-finder)
