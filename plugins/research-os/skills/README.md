...
- **Knowledge:** `wiki-setup`, `add-vault`, `wiki-pull`, `wiki-push`, `wiki-ingest`, `wiki-maintain`, `connect` (the wiki skills were vendored here — previously split across a separate `claude-global/` sync mechanism, retired once research-os itself became the global plugin install; `connect` is the read-only cross-theme bridge-finder)
- **Routines:** `daily-summary`, `weekly-planning`
- **Maintenance:** `check-update-upstream-repos`
- **Learning (vendored engram):** `learn` (engram's teaching loop, vendored directly, with research-os's context-sourced intake + wiki/project-journal linkage woven in), `recall` (engram's review loop, vendored, renamed to avoid colliding with `/peer-review`), `coach` (engram's telemetry/strategy/dashboard loop, vendored) — the engine (`scripts/engram.py`) and its three agents live in this plugin, not a separate install
