...
| `observe.py` | PreToolUse, PostToolUse | `Read\|Write\|Edit\|MultiEdit\|Bash\|Glob\|Grep` | **Off by default** — no-ops unless `observer-config.json` sets `enabled: true`. When on, appends tool-use metadata to a per-project JSONL under `ecc-homunculus/` for procedure-discovery mining. Every path goes through `scripts/capture_policy.py` first: data-file content is never captured, denied paths are recorded only as `<redacted:code>`. See the AD-5 canary (`capture_policy.py --selftest`). |
| `data-consent.py` | PreToolUse, PostToolUse | `Read\|Glob\|Grep` | R2 — "ask before reading unfamiliar data, remember the ruling." A real gate, not a nudge: a data-file extension not yet in `_brain/read-consent.yaml` returns `permissionDecision: "ask"` (a genuine permission prompt); a path already on the project's own AD-5-style deny-list is `"deny"`, unconditionally, never `"ask"` — approving past a hard deny would defeat the deny-list. PostToolUse records the grant only after the call actually succeeded. Code/docs are never gated. |
