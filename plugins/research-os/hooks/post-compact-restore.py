...
def read_passport_stage(project_dir: str) -> str | None:
    """Read the current pipeline stage from passport.yaml (regex, no PyYAML dep)."""
...
def format_restoration_message(
    ...
    if passport_stage:
        lines.append(f"{GREEN}passport.yaml pipeline stage:{NC} {passport_stage}")
        lines.append("")
    ...
    lines.append("  1. Run /research-os-help — it reads passport.yaml and names the next step")
...
def main() -> int:
    ...
    if pre_compact_state or plan_info or session_log or passport_stage:
        message = format_restoration_message(pre_compact_state, plan_info, session_log, passport_stage)
        print(message)
